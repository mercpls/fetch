/* globals chai setup suite test assert */
...
suite('polyfill', function () {
  ...
  suite('Request', function () {
    ...
    test('clone with used Request body', function () {
      var req = new Request('https://fetch.spec.whatwg.org/', {
        method: 'post',
        body: 'I work out'
      })
  ...
  suite('Body mixin', function () {
    ...
    featureDependent(suite, support.formData, 'formData', function () {
      ...
      test('formData rejects after body was consumed', function () {
        return fetch('/json')
          .then(function (response) {
    ...
    suite('aborting', function () {
      ...
      test('initially aborted signal', function () {
        ...
        return fetch('/request', {
          ...
          function (error) {
            if (!IEorEdge) assert.instanceOf(error, DOMException)
